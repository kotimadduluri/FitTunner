...
import com.fittuner.databinding.ItemRunBinding
...
class RunAdapter:
  ...
  //  private val mDiffer: AsyncListDiffer<RunTrack> = AsyncListDiffer<RunTrack>(this, RunAdapterDiffUtill())
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int)
            =RunViewHolder(
        DataBindingUtil.inflate(LayoutInflater.from(parent.context), R.layout.item_run, parent, false))
...
class RunViewHolder(val itemRunBinding: ItemRunBinding):RecyclerView.ViewHolder(itemRunBinding.root){
    lateinit var run: RunTrack
    fun bind(item: RunTrack) {
        run=item
        itemRunBinding.run=item
        itemRunBinding.executePendingBindings()
        with(itemView){
